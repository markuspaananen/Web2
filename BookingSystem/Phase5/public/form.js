...
function buildValidationMessage(errors) {
  ...
  return `Your request contains invalid or missing information. Please review the following fields:${lines.join("\n")}Correct the fields above and submit the form again.`;
  ...
  try {
    ...
    if (!response.ok) {
      ...
      if (response.status === 409) {
        ...
        const name = payload.resourceName || "This resource";
...
*/
    ...
    const msgLines = [];
    msgLines.push("Booking successfully created.");
    msgLines.push("");

    msgLines.push(`Booking Name: ${body?.data?.name ?? ""}`);
    //msgLines.push(`Resource ID: ${body?.data?.id ?? ""}`);

    //if (createdAt) {
    //  msgLines.push(`Created: ${createdAt}`);
    //}
